...
const countNumberOfChildren = (arr) => {
  ...
  return arr.reduce((acc, char) => {
    if (char.children) {
      let x = acc + char.children.length
      return x
    } else {
      return acc
    }
...
const extractStat = (statName, arr) => {
  ...
  console.log('see the object', object);
  if (object === {}) {
    return null
...
const extractChildren = (arr) => {
    // Solution code here...
    let newArray = arr.filter(obj => obj.name.toUpperCase().includes('A'))


    return newArray.reduce((acc, obj) => {
      if (obj.children) {
        obj.children.forEach(value => acc.push(value))
        return acc
      } else { return acc }
    }, [])
